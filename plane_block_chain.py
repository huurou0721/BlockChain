...
class Block:
    ...
    def check_valid_hash(self) -> bool:
        return int(self.calc_blockhash(), 16) <= self.calc_target()


class Blockchain:
    def __init__(self, initial_bits) -> None:
        self.chain: Sequence[Block] = []
    ...
    def getblockinfo(self, index=-1):
        return print(json.dumps(self.chain[index].to_json(),
                                indent=2,
                                sort_keys=True,
                                ensure_ascii=False))

    def mining(self, block: "Block") -> None:
